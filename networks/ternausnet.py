...
class ConvLayer(nn.Module):
    def __init__(self, in_features, out_features, relu=True):
        ...
        layers = list()
        layers.append(nn.Conv2d(in_features, out_features, kernel_size=3, stride=1, padding=1, bias=False))
        if relu:
...
class TernausNet(nn.Module):
    def __init__(self, num_class):
        ...
        for i in range(5):
            upconv_layers.append(nn.ConvTranspose2d(upsample_spec[i][0], upsample_spec[i][1], kernel_size=2, stride=2))
            relu = i != 4
            decoder_conv_layers.append(ConvLayer(decoder_conv_spec[i][0], decoder_conv_spec[i][1], relu=relu))
    ...
    def forward(self, x):
        ...
        features.reverse()
        for feature, upconv_layer, decoder_conv_layer in zip(features, self.upconv_layers, self.decoder_conv_layers):
            x = upconv_layer(x)
